...
static void usage(int status, const char *program_name)
{
    ...
    {
        dbg_time("Upgrade w's modules with Qualcomm's firehose protocol.\n");
...
static char * find_firehose_mbn(char** firehose_dir, size_t size)
{
    ...
    if (!wfile_find_file(*firehose_dir, "prog_nand_firehose_", ".mbn", &firehose_mbn)
        && !wfile_find_file(*firehose_dir, "prog_emmc_firehose_", ".mbn", &firehose_mbn)
        && !wfile_find_file(*firehose_dir, "prog_firehose_", ".mbn", &firehose_mbn)
        && !wfile_find_file(*firehose_dir, "prog_firehose_", ".elf", &firehose_mbn)
        && !wfile_find_file(*firehose_dir, "firehose-prog", ".mbn", &firehose_mbn)
        && !wfile_find_file(*firehose_dir, "prog_", ".mbn", &firehose_mbn)
        && !wfile_find_file(*firehose_dir, "xbl_s_devprg_ns_SA52X", ".melf", &firehose_mbn)  //AG590ECNABR01A01M8G_OCPU_01.001.01
        && !wfile_find_file(*firehose_dir, "xbl_s_devprg_ns", ".melf", &firehose_mbn)
     ) {
...
#endif
{
    ...
    umask(0);
    /*build V1.0.0*/
    dbg_time("Version: wfirehose_V1.0.0\n"); //when release, rename to V1.X
#ifndef __clang__
    ...
    while ( -1 != (opt = getopt(argc, argv, "f:p:z:s:l:u:d:nevh"))) {
        switch (opt) {
            ...
            case 'l':
                ...
                snprintf(filename, sizeof(filename), "%.80s/wfirehose_%lu.log", optarg, time(NULL));
...
_usb2tcp_start:
    ...
    if (module_sys_path[0] == '\0' && module_port_name[0] != '\0') {
        ...
        w_get_syspath_name_by_ttyport(module_port_name, module_sys_path, MAX_PATH);
    }

    if (module_sys_path[0] == '\0') {
        int module_count = auto_find_w_modules(module_sys_path, MAX_PATH, NULL, NULL);
        if (module_count <= 0) {
            dbg_time("w module not found\n");
            update_transfer_bytes(-1);
            error_return();
        }
            ...
            dbg_time("There are multiple w modules in system, Please use <-s /sys/bus/usb/devices/xx> specify which module you want to upgrade!\n");
    ...
    while (edl_retry-- > 0) {
        ...
        if (usb_handle) {
            ...
            if (usb3_speed >= 5000 && access(module_sys_path, R_OK) && errno_nodev()) {
                if (auto_find_w_modules(module_sys_path, MAX_PATH, "5c6/9008/", &usb3_atime) > 1) {
                    dbg_time("There are multiple w EDL modules in system!\n");
                    update_transfer_bytes(-1);
                    error_return();
                }
            }
