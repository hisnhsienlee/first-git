...
int firehose_main(const char *firehose_dir, void *usb_handle, unsigned qusb_zlp_mode) {
    ...
    if (q_module_packet_sign)
    {
        for (x = 0; x < 10; x++) {
            ...
            if (!wfile_find_file(firehose_dir, xmlfile_tmp, ".xml", &xmlfile_list[xmlfile_cnt])) {
                continue;
            }
            xmlfile_cnt++;
        }
    ...
    {
        if (!wfile_find_file(firehose_dir, "rawprogram_", ".xml", &xmlfile_list[xmlfile_cnt])
            && !wfile_find_file(firehose_dir, "firehose-rawprogram", ".xml", &xmlfile_list[xmlfile_cnt])
           ) {
    ...
    if (!wfile_find_file(firehose_dir, "patch_", ".xml", &xmlfile_list[xmlfile_cnt])
        && !wfile_find_file(firehose_dir, "patch-", ".xml", &xmlfile_list[xmlfile_cnt])
       ) {
        ...
        xmlfile_cnt++;
        dbg_time("11111111111111111111111\n");
    }
    ...
    if (!q_erase_all_before_download) {
        for (x = 0; x < fh_data->fh_cmd_count; x++) {
            ...
            if (fh_cmd->erase.SECTOR_SIZE_IN_BYTES == 0)  //��ֹBG95 ����¼ jira id: STMDM9205-5237 ����fh_cmd->erase.num_partition_sectors == 0�� ��Ϊ<erase SECTOR_SIZE_IN_BYTES="512" label="erase whole disk" physical_partition_number="0" start_sector="0" /> ��Ҫд��ģ��
                continue;
