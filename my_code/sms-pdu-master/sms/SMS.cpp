...
void sms_init()
{
    ...
    mRD = false;
    mSRR = false;
    mSCA = const_cast<char *>("");;
    mVP = const_cast<char *>("FF");;
    mCSMIEI = BIT8MIEI;
}
...
struct PDUS *PDUEncoding(const char *DA, const char *UDC, struct UDHS *udhs) {
    enum EnumDCS DCS;

    sms_init();

    if (isGSMString((char *)UDC))
        DCS = BIT7;
    else
        DCS = UCS2;
...
struct PDUS *PDUDoEncoding(const char *SCA, const char *DA, const char *UDC, struct UDHS *udhs, enum EnumDCS DCS) {
    // 短信拆分
...
struct UDS *UDCSplit(const char *UDC, struct UDHS *udhs, enum EnumDCS DCS) {
    ...
    if (DCS == BIT7) {
        ...
        // 不需要拆分
        if (SeptetsLength((char*)UDC) <= room) {
            result = (struct UDS *) malloc(sizeof(struct UDS));
            result->Data = (char **)malloc(sizeof(char *));
            result->total = 1;
            result->Data[0] = (char*)UDC;
        ...
        else // 拆分短信
        {
            ...
            while (i < len) {
                int step = SeptetsToChars((char *)UDC, i, room);
                if (i + step < len) {
...
int SeptetsToChars(const char *source, int index, int septets) {
    if (source == NULL || strcmp(source, "") == 0)
        return 0;
    int count = 0;
    int i;
...
char *SoloPDUEncoding(const char *SCA, const char *DA, const char *UC, struct UDHS *udhs, enum EnumDCS DCS) {
    ...
    // 用户数据长度及内容
    ret = UDEncoding((char *)UC, udhs, DCS);
    sprintf(buf, "%s", ret);
